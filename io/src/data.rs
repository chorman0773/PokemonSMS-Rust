...
use std::{fmt::Display, io::{Read,Write}, slice};
...
pub fn validate<T,U,E: StdError+'static,F: FnOnce(T)->std::result::Result<U,E>>(input: T,f: F) -> Result<U>{
...
impl Deserializeable for i8{
    fn deserialize<R: DataInput + ?Sized>(&mut self,input: &mut R) -> Result<()> {
        ...
        input.read_fully(slice::from_mut(unsafe{&mut *(self as *mut i8 as *mut u8)}))
    }
}
...
impl Deserializeable for String{
    fn deserialize<R: DataInput + ?Sized>(&mut self,input: &mut R) -> Result<()> {
        ...
        *self = validate(vec, |v|String::from_utf8(v))?;
        Ok(())
    }
}
...
impl DeserializeCopy for String{
    fn deserialize_copy<R: DataInput + ?Sized>(input: &mut R) -> Result<Self> {
        ...
        let mut vec = vec![0u8;size];
        input.read_fully(&mut vec)?;
        validate(vec, |v|String::from_utf8(v))
    }
}
...
impl<T: Display> Display for OutOfRange<T>{
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.write_fmt(format_args!("{} is not in range for this operation",&self.0))
    }
}
